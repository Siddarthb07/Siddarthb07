# Siddarth Boggarapu

> I build systems that operate under uncertainty across finance, healthcare, security, and physical simulation. My work combines machine learning, backend engineering, and physics-based modeling to create systems that are measurable, explainable, and grounded in real-world constraints.

AI · Backend Engineering · Aerospace Simulation · Security · Quantitative Finance

📍 Bangalore, India | 📧 <siddarthb078@gmail.com>

---

## Best Projects

| Project | Description | Stack |
| --- | --- | --- |
| 🧠 **[Anima](https://github.com/Siddarthb07/Anima)** | Dimensional LLM readouts — research tooling for valence / arousal / uncertainty probes via FastAPI. | FastAPI · Python · PyTorch |
| 🛰 **[Corvex](https://github.com/Siddarthb07/corvex)** | Multi-host campaign correlator — stitches weak signals across machines into one attack timeline. Sealed held-out eval; live containment gated behind a safety checklist and dry-run only. | Python · Docker |
| 🫀 **[Drift](https://github.com/Siddarthb07/Health-AI)** | Explainable health risk platform with clinically validated scoring (ACC/AHA, FINDRISC), 17-biomarker lab tracking, OAuth smartwatch sync. | Flask · Python · SQLite |
| 📈 **[GeoQuant](https://github.com/Siddarthb07/GeoQuant)** | Systematic trading pipeline — ML signals, news sentiment, Alpaca paper routing, self-learning retrain loop. | FastAPI · PyTorch · Alpaca API |
| 🌀 **[Drone Vortex Ring Simulation](https://github.com/Siddarthb07/Drone-Vortex-Ring-Simulation)** | Interactive vortex-ring physics under drone thrust profiles. Feeds the [NeuralVortex](https://github.com/Siddarthb07/NeuralVortex) ML surrogate. | Python · NumPy · Matplotlib |

---

## Aerospace cluster

Self-driven CFD / simulation work (and a short IISc summer internship on vortex rings):

- [vortex-tracker](https://github.com/Siddarthb07/vortex-tracker) — OpenCV measurement of vortex rings from high-speed sequences (built during 10-day IISc internship, May 2025)
- [Propeller-simulator](https://github.com/Siddarthb07/Propeller-simulator) — BEMT propeller sweeps
- [Drone-Vortex-Ring-Simulation](https://github.com/Siddarthb07/Drone-Vortex-Ring-Simulation) — reduced-order vortex dynamics
- [NeuralVortex](https://github.com/Siddarthb07/NeuralVortex) — Fourier Neural Operator surrogate on own CFD data

---

## Experience

**Summer Intern — Indian Institute of Science (IISc), Bangalore** `May 2025 · 10 days`
Vortex-ring internship. CFD / simulation repos above are self-driven personal work that predates and continues independently of IISc.

**Engineering Intern — Vegam Solutions** `Q3 2025 · Q1 2026`
Air-filtration engineering (Q3 2025); text-to-SQL RAG pipeline (Q1 2026, NDA). Public clean-room rebuild: [text2sql-rag](https://github.com/Siddarthb07/text2sql-rag).

**Founder — Athera** `Ongoing`
AI automation and workflows for small businesses — [athera.digital](https://athera.digital).

**Co-founder — ORQIS AI** `Ongoing`
AI agent monitoring and auto-remediation — catches silent agent failures (e.g. tool-call loops) and fixes them with human approval.

---

## A Few Other Things

- 🚁 Built and tuned quadcopter and hexacopter systems — flight control, propeller optimization, telemetry
- 🏠 Raspberry Pi NAS, DNS ad blocker, distributed computing experiments — [homelab-rpi](https://github.com/Siddarthb07/homelab-rpi)
- ⚽ Competitive skating, football, badminton, swimming · Model UN · 50+ hrs community service

---

[Portfolio](https://siddarthb07.github.io/siddarthb/) · [Corvex](https://github.com/Siddarthb07/corvex) · [Anima](https://github.com/Siddarthb07/anima). [Email](mailto:siddarthb078@gmail.com)
